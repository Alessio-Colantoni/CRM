    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        if (password == null) {
            if (existsAndIsConsistent(username)) {
                logger.info("Utente amministratore iniziale gia presente; password bootstrap non configurata: {}", username);
                return;
            }
            throw new IllegalStateException(
    ...
    private void createIfMissing(String username, String password, String name, String surname) throws SQLException {
        try (Connection connection = ConnectionFactory.getConnection()) {
            ...
            try {
                ...
                if (credentials == 1 && users == 1) {
                    connection.commit();
                    logger.info("Utente amministratore iniziale gia presente: {}", username);
                    return;
                }
                if (credentials != 0 || users != 0) {
                    throw new SQLException("Dati bootstrap amministratore incoerenti per " + username);
                ...
                connection.commit();
                logger.info("Creato utente amministratore iniziale: {}", username);
